...
describe('/_authenticated/tecnico route guard', () => {
  beforeEach(() => {
    vi.resetModules()
  })

  it('deja pasar a un usuario con rol tecnico', async () => {
  ...
  it('redirige a /403 a un usuario con rol cliente', async () => {
    const { useAuthStore, Route } = await importGuard()
  ...
  it('redirige a /403 cuando no hay usuario en el store', async () => {
    const { Route } = await importGuard()
